...
class Program
{
    ...
    private static char[][] executeQueenCommand(char[][] floor, int symbolRow, int symbolCol, int moveRow, int moveCol)
    {
        ...
        int min = Math.Min(symbolRow, symbolCol);
        bool onDiagonal = checkIsOnDiagonals(floor, symbolRow - min, symbolCol - min, symbolRow + min, symbolCol - min, moveRow, moveCol);
        if (!onDiagonal)
        {
            Console.WriteLine("Invalid move!");
            return floor;
    ...
    private static bool checkIsOnDiagonals(char[][] floor, int startRow, int startCol, int secondStartRow, int secondStartCol, int moveRow, int moveCol)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            if (startRow + i == moveRow && startCol + i == moveCol)
            {
                return true;
            }
            if (secondStartRow - i == moveRow && secondStartCol == moveCol)
            {
                return true;
            }
        }
    ...
    {
        if (symbolCol >= 0 && symbolCol <= 7 && symbolRow >= 0 && symbolRow <= 8)
        {
            return true;
        }
        return false;
